...
EXCHANGE_IDS = env_list("EXCHANGES", "gateio,mexc,kucoin,okx,bybit,bitget")
...
CMC_CATEGORIES = env_list(
    "CMC_CATEGORIES",
    "artificial-intelligence,cloud-computing,storage"
)
...
def build_exchanges() -> Dict[str, object]:
    exchanges = {}
    ...
    for ex_id in EXCHANGE_IDS:
        try:
            ...
            log.info("Loaded exchange: %s", ex_id)

        except AttributeError:
            log.warning("Exchange not supported by ccxt: %s", ex_id)

        except Exception as e:
            log.warning("Failed to load exchange %s: %s", ex_id, e)

    return exchanges


def symbols_from_cmc(exchange, cmc_symbols: Dict[str, Dict]) -> List[str]:
    symbols = []
...
def build_signal_message(signal: Dict, cmc_info: Optional[Dict]) -> str:
    ...
    exchange_name = signal.get("exchange", "").upper()
...
def validate_config():
    ...
    if not EXCHANGE_IDS:
        log.warning("EXCHANGES is empty.")
...
def main():
    ...
    if not exchanges:
        log.error("No exchanges loaded. Check EXCHANGES variable.")
        return
    ...
    if SEND_STARTUP_MESSAGE:
        send_telegram(
            ...
            f"المنصات: {', '.join([x.upper() for x in exchanges.keys()])}\n"
            f"الفريم: {TIMEFRAME}\n"
            "الإشارات: شراء فقط\n"
            ...
            if not cmc_symbols or now - last_cmc_refresh >= CMC_REFRESH_HOURS * 3600:
                ...
                for ex_id, exchange in exchanges.items():
                    try:
                        ...
                        pairs = symbols_from_cmc(exchange, cmc_symbols)

                        exchange_pairs[ex_id] = pairs

                        log.info("%s pairs matched: %s", ex_id, len(pairs))

                    except Exception as e:
                        log.warning(
                            "Could not refresh markets for %s: %s",
                            ex_id,
                            e
                        )

                        exchange_pairs[ex_id] = []

                last_cmc_refresh = now

            for ex_id, exchange in exchanges.items():
                pairs = exchange_pairs.get(ex_id, [])

                if not pairs:
                    log.warning("No pairs matched for %s", ex_id)
                    continue
                ...
                for pair in pairs:
                    ...
                    signal["exchange"] = ex_id

                    signal_key = (
                        f"{ex_id}|{pair}|{TIMEFRAME}|"
                        f"{signal['candle_timestamp']}|BUY"
                    )
                    ...
                    msg = build_signal_message(
                        signal,
                        cmc_symbols.get(base, {})
                    )

                    ok = send_telegram(msg)

                    if ok:
                        state["sent_signals"][signal_key] = (
